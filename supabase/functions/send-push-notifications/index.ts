...
Deno.serve(async (request) => {
  ...
  if (!workerSecret || request.headers.get("x-haajar-worker-secret") !== workerSecret) {
    return new Response("Unauthorized", { status: 401 });
  }

  const supabaseUrl = Deno.env.get("SUPABASE_URL");
  const serviceRoleKey = Deno.env.get("SUPABASE_SERVICE_ROLE_KEY");
  if (!supabaseUrl || !serviceRoleKey) return new Response("Server configuration unavailable", { status: 500 });

  const supabase = createClient(supabaseUrl, serviceRoleKey, {
    auth: { autoRefreshToken: false, persistSession: false },
  });
  const { data, error } = await supabase.rpc("claim_push_deliveries", { batch_size: 100 });
  if (error) return Response.json({ processed: 0, status: "claim_failed" }, { status: 500 });
  ...
  for (const delivery of (data ?? []) as ClaimedDelivery[]) {
    try {
      ...
      const status = response.ok && ticket?.status === "ok" ? "sent" : invalidToken ? "invalid_token" : "failed";
      await supabase.rpc("complete_push_delivery", {
        target_delivery_id: delivery.delivery_id,
        delivery_status: status,
        provider_ticket: ticket?.id ?? null,
        error_code: ticket?.details?.error ?? (response.ok ? null : `HTTP_${response.status}`),
      });
